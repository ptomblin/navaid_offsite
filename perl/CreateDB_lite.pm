...
BEGIN
{
    push @INC, "/config_backup/navaid.com/perl";
}
...
my $conn;
$conn = DBI->connect(
        "DBI:SQLite:dbname=/config_backup/navaid.com/nav_data/waypoints.db",
        "", "") or die $conn->errstr;
$conn->{"AutoCommit"} = 0;
$conn->func( 3600000, 'busy_timeout' );
...
my $runwaysStmt = $conn->prepare(
        "SELECT     runway_designation, length, width, surface, " .
                   "b_lat, b_long, e_lat, e_long  " .
        "FROM       runways " .
        "WHERE      datasource_key = ? and not closed " .
...
sub generate($$$$)
{
    ...
    my @datasources = @{$paramRef->{datasources}};
    if (scalar(@datasources) < 1)
    {
        @datasources = (1, 2);
    }
    ...
    my $datasource_string = " in (" . join(",",@datasources) . ")";

    &$debugCodeRef("datasources = $datasource_string\n");

    $result = $conn->prepare(
            "SELECT     ds_index, source_name, source_long_name, source_url, " .
            "			credit, available_types, updated " .
            "FROM       datasource " .
            "WHERE		ds_index $datasource_string")
    or die $conn->errstr;
    ...
    {
        &$debugCodeRef("doing datasource $ds\n");

        &$datasourceHashCodeRef($datasource_hash{$ds});

        $bigSelect->execute($ds)
        or die $conn->errstr;
        ...
        while (@row = $bigSelect->fetchrow_array)
        {
            my ($id, $pdb_id, $datasource_key, $type, $name, $address, $state, 
                ...
                $main_frequency, $elevation, $category, $tpa) = @row;
            ...
            $record{datasource} = $datasource_hash{$ds}->{source_name};
