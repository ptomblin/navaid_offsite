...
sub _load_records
{
	...
	for ($i = 0; $i < $pdb->{_numrecs}; $i++)
	{
		...
		$record = $pdb->ParseRecord(
			%{$pdb->{_index}[$i]},
			"data"	=> $buf,
            "index" => $i, # PT Add index so that I know what record I'm on
			);
...
sub Write
{
	...
	if ($self->{attributes}{resource} || $self->{'attributes'}{'ResDB'})
	{
        ...
        # ##PT## Changed locally to save some memory
        while (defined($resource = shift(@{$self->{resources}})))
		{
			my $type;
			my $id;
			my $data;
        ...
        while (defined($record = shift(@{$self->{records}})))
		{
            ...
            $record->{"index"} = $index++; # PT Send in the record number
