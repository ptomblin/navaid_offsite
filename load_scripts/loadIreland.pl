...
while (<$fh>)
{
	...
	if ($rtype eq "AIRPORT")
	{
		...
		insertWaypoint($id, $datasource_key, $type, $name, "",
						...
						undef, Datasources::DATASOURCE_EI_AM, !$private,
                        0, 0);
	}
	elsif ($rtype eq "RWY")
	{
		...
		insertRunway($datasource_key, $designation,
            ...
            undef, undef, undef, undef,
			Datasources::DATASOURCE_EI_AM, 0);
	}
	elsif ($rtype eq "COMM")
	{
	...
	elsif ($rtype eq "NAVAID")
	{
		...
		insertWaypoint($id, $datasource_key, $rtype, $name, "",
						"", $country, $lat, $long, $decl, $elev,
						$freq, Datasources::DATASOURCE_EI_AM, 1, 0, 0);
	}
	elsif ($rtype eq "REP-PT")
	{
		...
		insertWaypoint($id, $datasource_key, $rtype, $name, "",
						...
						undef, Datasources::DATASOURCE_EI_AM, 1, 1, 0);
        $rep_pt_id = $id;
	}
	elsif ($rtype eq "DEFN")
	{
		...
		insertFix($rep_pt_id, $datasource_key, Datasources::DATASOURCE_EI_AM, 
            $navaid, $navaid_type, $radial, $distance, 0);
	}
    elsif ($rtype eq "NOTE")
    {
    }
